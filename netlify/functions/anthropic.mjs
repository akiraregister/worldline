...
//
// 必要な環境変数（Netlify の Site configuration → Environment variables）:
//   ANTHROPIC_API_KEY  … 必須。あなたの Anthropic の APIキー（sk-ant-...）。
//   ALLOWED_EMAILS     … 推奨。利用を許可するメールをカンマ区切りで（例: me@gmail.com,partner@gmail.com）。
//                        未設定だと「ログインした任意の人」が使えてしまうため、必ず設定を推奨。
